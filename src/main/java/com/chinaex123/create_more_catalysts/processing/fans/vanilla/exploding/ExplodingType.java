 ...
 * 批量爆炸鼓风机类型
 */
public final class ExplodingType extends FanCommonType {

    private static final int AIRFLOW_COLOR = 0x4A4A4A;
    private static final int EXPLODE_COLOR = 0xFF4500;
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.EXPLODING_ENTITY_TRANSFORMS.get();
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.EXPLODING_HEAL_ENTITIES.get();
    ...
    public ExplodingType() {
        super(FanRecipeType.EXPLODING);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量爆炸鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_EXPLODING_FAN.get();
    }
     ...
     * @param level 世界实例
     * @param pos 待检测的位置
     * @return 该位置是否为有效的苦力怕头催化剂位置
     */
    @Override
    public boolean isValidAt(Level level, BlockPos pos) {
        if (!isFanProcessingEnabled()) return false;
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_EXPLODING,
                ModBlockTags.FAN_CATALYSTS_EXPLODING
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, EXPLODE_COLOR, ParticleTypes.EXPLOSION);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, EXPLODE_COLOR, 0.06f, ParticleTypes.EXPLOSION);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        int cooldown = EXPLOSION_COOLDOWN.getOrDefault(living, 0);
        if (cooldown > 0) {
            EXPLOSION_COOLDOWN.put(living, cooldown - 1);
            return;
        }

        level.explode(null, living.getX(), living.getY() + living.getEyeHeight() * 0.5f, living.getZ(),
