 ...
 * 批量吐息鼓风机类型
 */
public final class BreathedWindType extends FanCommonType {

    private static final int AIRFLOW_COLOR = 0x8B00FF;
    private static final int BREATH_COLOR = 0xCC66FF;
     ...
     * 实体转换反馈效果
     */
    private static final FanEntityTransformHelper.TransformationFeedback BREATH_FEEDBACK =
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.BREATHED_WIND_ENTITY_TRANSFORMS.get();
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.BREATHED_WIND_HEAL_ENTITIES.get();
     ...
     * 构造函数
     */
    public BreathedWindType() {
        super(FanRecipeType.BREATHED_WIND);
    }
    ...
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_BREATHED_WIND_FAN.get();
    }
     ...
     * @param level 世界实例
     * @param pos 待检测的位置
     * @return 该位置是否为有效的龙首催化剂位置
     */
    @Override
    public boolean isValidAt(Level level, BlockPos pos) {
        if (!isFanProcessingEnabled()) return false;
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_BREATHED_WIND,
                ModBlockTags.FAN_CATALYSTS_BREATHED_WIND
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, BREATH_COLOR, ParticleTypes.DRAGON_BREATH);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, BREATH_COLOR, 0.04f, ParticleTypes.DRAGON_BREATH);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        if (outputType != null) {
            ...
            FanEntityTransformHelper.transformMob(level, living, mobType, BREATH_FEEDBACK);
            return;
        }
