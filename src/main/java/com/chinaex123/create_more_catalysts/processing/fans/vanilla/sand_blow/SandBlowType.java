 ...
 * 批量扬砂鼓风机类型
 */
public final class SandBlowType extends FanCommonType {

    private static final int AIRFLOW_COLOR = 0xC2B280;
    private static final int SAND_COLOR = 0xE6D5AC;
    ...
    private static final List<SandBlowType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.SAND_BLOW_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sand Blow]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.SAND_BLOW_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sand Blow]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.SAND_BLOW_POTION_EFFECTS.get();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new SandBlowType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sand Blow]Invalid potion effect config: {}", effect);
                }
            }
        }
    }
     ...
     */
    public SandBlowType() {
        super(FanRecipeType.SAND_BLOW);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量扬砂鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_SAND_BLOW_FAN.get();
    }
     ...
     * @param level 世界实例
     * @param pos 待检测的位置
     * @return 该位置是否为有效的沙子催化剂位置
     */
    @Override
    public boolean isValidAt(Level level, BlockPos pos) {
        if (!isFanProcessingEnabled()) return false;
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_SAND_BLOW,
                ModBlockTags.FAN_CATALYSTS_SAND_BLOW
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, SAND_COLOR, ParticleTypes.POOF);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, SAND_COLOR, 0.03f, ParticleTypes.POOF);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        // 为配置的实体施加状态效果
        for (SandBlowType.PotionEffectEntry entry : POTION_EFFECTS) {
        ...
        // 播放音效
        FanProcessingSounds.sandBlowSound(level, living.blockPosition());
    }
