 ...
 * 批量磺化鼓风机类型
 */
public final class SulfonType extends MekanismFanCommonType {

    private static final int AIRFLOW_COLOR = 0xFFFF00;
    private static final int SULFON_COLOR = 0xFFD700;
    ...
    private static final List<SulfonType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
     ...
     * 实体转换反馈效果
     */
    private static final FanEntityTransformHelper.TransformationFeedback SULFON_TRANSFORM_FEEDBACK =
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.SULFON_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sulfon]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.SULFON_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sulfon]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.SULFON_POTION_EFFECTS.get();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new SulfonType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sulfon]Invalid potion effect config: {}", effect);
                }
            }
        }
    }
    ...
    public SulfonType() {
        super(MekanismFanRecipeType.SULFON);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量磺化鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_SULFON_FAN.get();
    }
    ...
    public boolean isValidAt(Level level, BlockPos pos) {
        ...
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_SULFON,
                ModBlockTags.FAN_CATALYSTS_SULFON
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, SULFON_COLOR, ParticleTypes.SMOKE);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, SULFON_COLOR, 0.05f, ParticleTypes.SMOKE);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        if (outputType != null) {
            ...
            FanEntityTransformHelper.transformMob(level, living, mobType, SULFON_TRANSFORM_FEEDBACK);
            return;
        }
        ...
        // 为配置的实体施加状态效果
        for (SulfonType.PotionEffectEntry entry : POTION_EFFECTS) {
        ...
        // 播放音效
        FanProcessingSounds.sulfonSound(level, living.blockPosition());
    }
