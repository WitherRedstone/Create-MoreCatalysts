 ...
 * 批量盐析鼓风机类型
 */
public final class SaloutType extends MekanismFanCommonType {

    private static final int AIRFLOW_COLOR = 0xE8E8E8;
    private static final int SALT_COLOR = 0xFFFFFF;
    ...
    private static final List<SaloutType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
     ...
     * 实体转换反馈效果
     */
    private static final FanEntityTransformHelper.TransformationFeedback SALOUT_TRANSFORM_FEEDBACK =
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.SALOUT_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Salout]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.SALOUT_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Salout]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.SALOUT_POTION_EFFECTS.get();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new SaloutType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
                    CreateMoreCatalysts.LOGGER.warn("[Batch Salout]Invalid potion effect config: {}", effect);
                }
            }
        }
    }
    ...
    public SaloutType() {
        super(MekanismFanRecipeType.SALOUT);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量吐息鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_SALOUT_FAN.get();
    }
    ...
    public boolean isValidAt(Level level, BlockPos pos) {
        ...
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_SALOUT,
                ModBlockTags.FAN_CATALYSTS_SALOUT
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, SALT_COLOR, ParticleTypes.WHITE_ASH);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, SALT_COLOR, 0.05f, ParticleTypes.WHITE_ASH);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        if (outputType != null) {
            ...
            FanEntityTransformHelper.transformMob(level, living, mobType, SALOUT_TRANSFORM_FEEDBACK);
            return;
        }
        ...
        // 为配置的实体施加状态效果
        for (SaloutType.PotionEffectEntry entry : POTION_EFFECTS) {
        ...
        // 播放音效
        FanProcessingSounds.saloutSound(level, living.blockPosition());
    }
