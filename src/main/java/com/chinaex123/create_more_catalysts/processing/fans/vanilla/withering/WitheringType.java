 ...
 * 批量凋零风扇类型
 */
public final class WitheringType extends FanCommonType {

    private static final int AIRFLOW_COLOR = 0x200223;
     ...
     * 实体转换反馈效果
     */
    private static final FanEntityTransformHelper.TransformationFeedback SKELETON_WITHER_FEEDBACK =
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.WITHERING_ENTITY_TRANSFORMS.get();
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.WITHERING_HEAL_ENTITIES.get();
    ...
    public WitheringType() {
        super(FanRecipeType.WITHERING);
    }

    /**
     * 检查风扇加工是否启用：读取配置文件中的开关状态
     * @return 是否启用凋零风扇加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_WITHERING_FAN.get();
    }
     ...
     * @param level 世界实例
     * @param pos 待检测的位置
     * @return 该位置是否为有效的凋零催化剂位置
     */
    @Override
    public boolean isValidAt(Level level, BlockPos pos) {
        if (!isFanProcessingEnabled()) return false;
        return isValidBlockCatalyst(level, pos, ModBlockTags.FAN_CATALYSTS_WITHERING);
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, COATING_COLOR, ParticleTypes.SOUL);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, COATING_COLOR, 0.05f, ParticleTypes.SOUL);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        if (outputType != null) {
            ...
            FanEntityTransformHelper.transformMob(level, living, mobType, SKELETON_WITHER_FEEDBACK);
            return;
        }
