...
import com.chinaex123.create_more_catalysts.processing.fans.ImmersiveEngineeringFanCommonType;
...
/**
 * 批量防腐鼓风机类型
 * 使用杂酚油作为催化剂，防止生物腐烂和变质
 */
public final class PreservationType extends ImmersiveEngineeringFanCommonType {

    private static final int AIRFLOW_COLOR = 0x654321;
    private static final int PRESERVATION_COLOR = 0x8B6914;
     ...
     * 实体转换反馈效果
     */
    private static final FanEntityTransformHelper.TransformationFeedback PRESERVATION_TRANSFORM_FEEDBACK =
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.PRESERVATION_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Preservation]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.PRESERVATION_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Preservation]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.PRESERVATION_POTION_EFFECTS.get();
    ...
    public PreservationType() {
        super(ImmersiveEngineeringFanRecipeType.PRESERVATION);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量防腐鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_PRESERVATION_FAN.get();
    }
    ...
    public boolean isValidAt(Level level, BlockPos pos) {
        ...
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_PRESERVATION,
                ModBlockTags.FAN_CATALYSTS_PRESERVATION
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, PRESERVATION_COLOR, ParticleTypes.HAPPY_VILLAGER);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, PRESERVATION_COLOR, 0.05f, ParticleTypes.GLOW);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        if (outputType != null) {
            ...
            FanEntityTransformHelper.transformMob(level, living, mobType, PRESERVATION_TRANSFORM_FEEDBACK);
            return;
        }
        ...
        // 播放音效
        FanProcessingSounds.preservationSound(level, living.blockPosition());
    }
