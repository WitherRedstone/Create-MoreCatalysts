...
public final class TidalType extends FanCommonType {
    ...
    private static final List<TidalType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new TidalType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        // 为配置的实体施加状态效果
        for (TidalType.PotionEffectEntry entry : POTION_EFFECTS) {
