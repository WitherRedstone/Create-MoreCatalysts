 ...
 * 批量幽匿鼓风机类型
 */
public final class SculkingType extends FanCommonType {

    private static final int AIRFLOW_COLOR = 0x0D4F4F;
    private static final int SCULK_COLOR = 0x1A8B7E;
    ...
    private static final List<SculkingType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
    ...
    private static final FanEntityTransformHelper.TransformationFeedback DEFAULT_TRANSFORM_FEEDBACK =
            new FanEntityTransformHelper.TransformationFeedback(
                    ...
                    new SculkChargeParticleOptions(0.5f)
            );
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.SCULKING_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sculking]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.SCULKING_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sculking]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.SCULKING_POTION_EFFECTS.get();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new SculkingType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
                    CreateMoreCatalysts.LOGGER.warn("[Batch Sculking]Invalid potion effect config: {}", effect);
                }
            }
        }
    }
     ...
     */
    public SculkingType() {
        super(FanRecipeType.SCULKING);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量幽匿鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_SCULKING_FAN.get();
    }
     ...
     * @param level 世界实例
     * @param pos 待检测的位置
     * @return 该位置是否为有效的幽匿催化剂位置
     */
    @Override
    public boolean isValidAt(Level level, BlockPos pos) {
        if (!isFanProcessingEnabled()) return false;
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_SCULKING,
                ModBlockTags.FAN_CATALYSTS_SCULKING
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, SCULK_COLOR, new SculkChargeParticleOptions(0.5f));
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, SCULK_COLOR, 0.05f, new SculkChargeParticleOptions(0.5f));
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        // 为配置的实体施加状态效果
        for (SculkingType.PotionEffectEntry entry : POTION_EFFECTS) {
        ...
        // 播放音效
        FanProcessingSounds.sculkingSound(level, living.blockPosition());
    }
