...
public final class PurifyingType extends FanCommonType {
    ...
    private static final List<PurifyingType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
        ...
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Purifying]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
        ...
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Purifying]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.PURIFYING_POTION_EFFECTS.get();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new PurifyingType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
                    CreateMoreCatalysts.LOGGER.warn("[Batch Purifying]Invalid potion effect config: {}", effect);
                }
            }
        }
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        // 为配置的实体施加状态效果
        for (PurifyingType.PotionEffectEntry entry : POTION_EFFECTS) {
