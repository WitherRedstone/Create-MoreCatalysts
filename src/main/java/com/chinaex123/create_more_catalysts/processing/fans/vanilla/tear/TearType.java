 ...
 * 批量催泪鼓风机类型
 */
public final class TearType extends FanCommonType {

    private static final int AIRFLOW_COLOR = 0x6B4C9A;
    private static final int TEAR_COLOR = 0x9B59B6;
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.TEAR_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Tear]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.TEAR_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Tear]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.TEAR_POTION_EFFECTS.get();
        for (String effect : effects) {
            ...
            if (parts.length >= 3) {
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Tear]Invalid potion effect config: {}", effect);
                }
            }
        }
    }
     ...
     */
    public TearType() {
        super(FanRecipeType.TEAR);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量催泪鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_TEAR_FAN.get();
    }
     ...
     * @param level 世界实例
     * @param pos 待检测的位置
     * @return 该位置是否为有效的哭泣黑曜石催化剂位置
     */
    @Override
    public boolean isValidAt(Level level, BlockPos pos) {
        if (!isFanProcessingEnabled()) return false;
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_TEAR,
                ModBlockTags.FAN_CATALYSTS_TEAR
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, TEAR_COLOR, ParticleTypes.ENCHANT);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, TEAR_COLOR, 0.05f, ParticleTypes.ENCHANT);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        // 播放音效
        FanProcessingSounds.tearSound(level, living.blockPosition());
    }
