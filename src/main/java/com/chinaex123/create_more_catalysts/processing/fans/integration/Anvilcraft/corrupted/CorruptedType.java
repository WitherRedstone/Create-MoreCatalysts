...
import com.chinaex123.create_more_catalysts.processing.fans.AnvilcraftFanCommonType;
...
/**
 * 批量腐化鼓风机类型
 * 使用腐化信标/恶意信标作为催化剂，防止生物腐烂和变质
 */
public final class CorruptedType extends AnvilcraftFanCommonType {

    private static final int AIRFLOW_COLOR = 0x2D1B3D;
    private static final int CORRUPTED_COLOR = 0x4A0E4E;
    ...
    private static final List<CorruptedType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
     ...
     * 实体转换反馈效果
     */
    private static final FanEntityTransformHelper.TransformationFeedback CORRUPTED_TRANSFORM_FEEDBACK =
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.CORRUPTED_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Corrupted]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.CORRUPTED_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Corrupted]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.CORRUPTED_POTION_EFFECTS.get();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new CorruptedType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
                    CreateMoreCatalysts.LOGGER.warn("[Batch Corrupted]Invalid potion effect config: {}", effect);
                }
            }
        }
    }

    public CorruptedType() {
        super(AnvilcraftFanRecipeType.CORRUPTED);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量腐化鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_CORRUPTED_FAN.get();
    }
    ...
    public boolean isValidAt(Level level, BlockPos pos) {
        ...
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_CORRUPTED,
                ModBlockTags.FAN_CATALYSTS_CORRUPTED
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, CORRUPTED_COLOR, ParticleTypes.SOUL);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, CORRUPTED_COLOR, 0.05f, ParticleTypes.SOUL_FIRE_FLAME);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        if (outputType != null) {
            ...
            FanEntityTransformHelper.transformMob(level, living, mobType, CORRUPTED_TRANSFORM_FEEDBACK);
            return;
        }
        ...
        // 为配置的实体施加状态效果
        for (CorruptedType.PotionEffectEntry entry : POTION_EFFECTS) {
        ...
        // 播放音效
        FanProcessingSounds.corruptedSound(level, living.blockPosition());
    }
