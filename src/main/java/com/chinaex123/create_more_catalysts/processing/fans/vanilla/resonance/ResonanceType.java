 ...
 * 批量共振鼓风机类型
 */
public final class ResonanceType extends FanCommonType {

    private static final int AIRFLOW_COLOR = 0x9B59B6;
    private static final int RESONANCE_COLOR = 0xE8A0E8;
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.RESONANCE_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Resonance]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.RESONANCE_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Resonance]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.RESONANCE_POTION_EFFECTS.get();
    ...
    public ResonanceType() {
        super(FanRecipeType.RESONANCE);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量共振鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_RESONANCE_FAN.get();
    }
     ...
     * @param level 世界实例
     * @param pos 待检测的位置
     * @return 该位置是否为有效的紫水晶簇催化剂位置
     */
    @Override
    public boolean isValidAt(Level level, BlockPos pos) {
        if (!isFanProcessingEnabled()) return false;
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_RESONANCE,
                ModBlockTags.FAN_CATALYSTS_RESONANCE
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, RESONANCE_COLOR, ParticleTypes.ELECTRIC_SPARK);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, RESONANCE_COLOR, 0.04f, ParticleTypes.ELECTRIC_SPARK);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        // 播放音效
        FanProcessingSounds.resonanceSound(level, living.blockPosition());
    }
