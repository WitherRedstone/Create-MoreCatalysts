 ...
 * 批量脱水鼓风机类型
 */
public final class DehydType extends MekanismFanCommonType {

    private static final int AIRFLOW_COLOR = 0x8B4513;
    private static final int DEHYD_COLOR = 0xD2691E;
    ...
    private static final List<DehydType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
     ...
     * 实体转换反馈效果
     */
    private static final FanEntityTransformHelper.TransformationFeedback DEHYD_TRANSFORM_FEEDBACK =
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.DEHYD_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Dehyd]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.DEHYD_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Dehyd]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.DEHYD_POTION_EFFECTS.get();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new DehydType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
    ...
    public DehydType() {
        super(MekanismFanRecipeType.DEHYD);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量脱水鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_DEHYD_FAN.get();
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, DEHYD_COLOR, ParticleTypes.SMOKE);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, DEHYD_COLOR, 0.05f, ParticleTypes.SMOKE);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        if (outputType != null) {
            ...
            FanEntityTransformHelper.transformMob(level, living, mobType, DEHYD_TRANSFORM_FEEDBACK);
            return;
        }
        ...
        // 为配置的实体施加状态效果
        for (DehydType.PotionEffectEntry entry : POTION_EFFECTS) {
        ...
        // 播放音效
        FanProcessingSounds.dehydSound(level, living.blockPosition());
    }
