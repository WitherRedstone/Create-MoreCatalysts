 ...
 * 批量发酵鼓风机类型
 */
public final class FermentType extends MekanismFanCommonType {

    private static final int AIRFLOW_COLOR = 0x9ACD32;
    private static final int FERMENT_COLOR = 0x7CFC00;
    ...
    private static final List<FermentType.PotionEffectEntry> POTION_EFFECTS = new ArrayList<>();
     ...
     * 实体转换反馈效果
     */
    private static final FanEntityTransformHelper.TransformationFeedback FERMENT_TRANSFORM_FEEDBACK =
    ...
    private static void loadTransformConfig() {
        List<? extends String> transforms = CommonConfig.FERMENT_ENTITY_TRANSFORMS.get();
        for (String transform : transforms) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Ferment]Invalid entity transform config: {}", transform);
                }
            }
        }
    }
    ...
    private static void loadHealConfig() {
        List<? extends String> heals = CommonConfig.FERMENT_HEAL_ENTITIES.get();
        for (String heal : heals) {
            ...
            if (parts.length == 2) {
                ...
                try {
                    ...
                    CreateMoreCatalysts.LOGGER.warn("[Batch Ferment]Invalid heal amount for {}: {} (must be a number)", entityName, healValueStr);
                }
            }
        }
    }
    ...
    private static void loadPotionEffectsConfig() {
        ...
        List<? extends String> effects = CommonConfig.FERMENT_POTION_EFFECTS.get();
                ...
                try {
                    ...
                    if (effectLoc != null) {
                        BuiltInRegistries.MOB_EFFECT.getHolder(effectLoc).ifPresent(eff ->
                                POTION_EFFECTS.add(new FermentType.PotionEffectEntry(eff, duration, amplifier)));
                    }
                } catch (NumberFormatException e) {
                    CreateMoreCatalysts.LOGGER.warn("[Batch Ferment]Invalid potion effect config: {}", effect);
                }
            }
        }
    }
    ...
    public FermentType() {
        super(MekanismFanRecipeType.FERMENT);
    }

    /**
     * 检查鼓风机加工是否启用
     * @return 是否启用批量发酵鼓风机加工
     */
    @Override
    protected boolean isFanProcessingEnabled() {
        return CommonConfig.ENABLE_FERMENT_FAN.get();
    }
    ...
    public boolean isValidAt(Level level, BlockPos pos) {
        ...
        return isValidCatalyst(level, pos,
                ModFluidTags.FAN_CATALYSTS_FERMENT,
                ModBlockTags.FAN_CATALYSTS_FERMENT
        );
    }
    ...
    public void spawnProcessingParticles(Level level, Vec3 pos) {
        spawnDustWithParticle(level, pos, FERMENT_COLOR, ParticleTypes.HAPPY_VILLAGER);
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        morphStandardAirFlow(particleAccess, random, AIRFLOW_COLOR, FERMENT_COLOR, 0.05f, ParticleTypes.HAPPY_VILLAGER);
    }
    ...
    protected void affectLivingEntity(LivingEntity living, Level level) {
        ...
        if (outputType != null) {
            ...
            FanEntityTransformHelper.transformMob(level, living, mobType, FERMENT_TRANSFORM_FEEDBACK);
            return;
        }
        ...
        // 为配置的实体施加状态效果
        for (FermentType.PotionEffectEntry entry : POTION_EFFECTS) {
        ...
        // 播放音效
        FanProcessingSounds.fermentSound(level, living.blockPosition());
    }
